...
<input type="range" id="disp" min="0" max="12000" step="250" value="6130"
 autocomplete="off">
...
<p>Both bootstrap notebooks run the whole pipeline on a free T4 in about ten
minutes and write every number in these charts.
<code>demos/gradio_app.py</code> streams eager against graphed decode side by
side with live counters; it needs a GPU, so run it from a notebook with
<code>--share</code>.</p>
